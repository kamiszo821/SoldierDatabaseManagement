#pragma once
class Szeregowy :public Soldier
{
	string rank;
	string role;
	string name;
...
public:
	...
	Szeregowy(string RANK,string ROLE,string NAME,string SURNAME,int YEARS_OF_SERVICE,string BIRTH) : rank(RANK), role(ROLE), name(NAME), surname(SURNAME), years_of_service(YEARS_OF_SERVICE), birth(BIRTH) {}
	~Szeregowy() {};
};
