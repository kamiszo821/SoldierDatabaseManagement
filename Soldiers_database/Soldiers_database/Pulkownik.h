#pragma once
#include "Menu.h"
class Pulkownik :public Soldier
{
	string rank;
	string role;
	string name;
...
public:
	...
	Pulkownik(string RANK, string ROLE, string NAME, string SURNAME, int YEARS_OF_SERVICE,string BIRTH) : rank(RANK), role(ROLE), name(NAME), surname(SURNAME), years_of_service(YEARS_OF_SERVICE), birth(BIRTH) {}
	~Pulkownik() {};
};
